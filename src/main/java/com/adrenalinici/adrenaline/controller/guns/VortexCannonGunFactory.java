...
public class VortexCannonGunFactory extends BaseEffectGunFactory {

  private static final TriConsumer<BaseEffectGunFlowState, GameModel, ControllerFlowContext> APPLY = (state, gameModel, context) -> {
    Position chosenCell = !state.getChosenCellsToHit().isEmpty() ?
    ...
    if (state.isActivatedFirstExtraEffect() && thirdPlayer != null) {
      state.hitPlayer(thirdPlayer, 1);
    }

    if (chosenCell != null)
      state.getChosenPlayersToHit().forEach(p -> gameModel.movePlayerInDashboard(chosenCell, p));
  };

  @Override
  public boolean canConsume(String key, ObjectNode config) {
    return "vortex_cannon".equals(key);
  }

  @Override
  public List<ControllerFlowNode> getAdditionalNodes(String key, ObjectNode config) {
    return Collections.singletonList(
      new ApplyBaseGunFlowNode(ControllerNodes.applyGunEffect("vortex_cannon", "base"), APPLY)
    );
  }
}
